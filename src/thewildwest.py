...
print("Howdy partner and welcome to the Wild, Wild, West. I'm Nancy Narrator!")
...
# The two lines of code below are used as a way to quit whenever you lose the game.
while do_they_want_to_play == "no":
    print("Loser.")
...
def run_tictactoe():
    ...
    while not(isBoardFull(board)):
        ...
        if not(isWinner(board, 'X')):
            move = compMove()
            if move == 0:
                print('Tie goes to you. Freaking meanie. Whatever, I guess I\'ll assassinate that guy for you. Long live the Wild West!!')
                do_they_want_to_play = 'no'

            else:
                insertLetter('O', move)
                print('I placed an \'O\' in position', move , ':')
                printBoard(board)
        else:
            print('I can\'t believe you beat me. Ugh. Whatever, I guess I\'ll assassinate that guy for you. Long live the Wild West!!')
            do_they_want_to_play = 'no'
            break
...
# The third portion of code that comes next is for the final level. Which is probably pretty disappointing to a first time player, as its literally just a 50/50 rng coin flip to kill Big Bill
# The reaason for the end being so difficult to win is because I don't want the game to be over on the first try...

def run_coinflip():
    coin_sides = [1,2]
    landed_coin = []
    alive = True
    coin_choice = input("Pick a side. [1] for Heads and [2] for tails.")
    while alive == True:
        landed_coin.append(coin_sides.pop())
        if landed_coin == coin_choice:
            print("You win.")
            break
        elif landed_coin != coin_choice:
            print("You lose.")
            alive = False
            do_they_want_to_play = 'no'
            break
...
if do_they_want_to_play == "no":
    ...
    print(f"\nNice to meet you, {player_name}. \nCrazy stuff's been going down in this ol' boom town as of late, eh?")
    ...
    print("\n Actually, that's irrelevant. Get out there and kick some conductor butt!")
    print("")
    print("")
    print("")
    ...
    print("*Chugga chugga chugga*")
    print("*Chugga chugga chugga*\n ")
    print("\n*BOOOOOOOOOOM*\n")
    print("\nNarrator: Congratulations!! You were able to take out the second henchman to Big Bill, Rockin' Ricky!")
    print("Narrator: All that's left is to kill the man himself! You have to hurry up, though, because I've arranged for you to meet him in a duel at town's center at High Noon!")
    print("Narrator: Get on it! You've only got 5 minutes left!\n")

    print("Big Bill: WELL! If it isn't the infamous sheriff out to get me! Hahahaha. Get ready to meet your maker.")
    print("Big Bill: But of course, I shall give you a fair chance. I'm not really a dueling man, myself, however, so we're going to leave this down to a coin toss. You get the call, and if you win, I skip town, but if I win, say goodbye because it's time to die!")

    run_coinflip()

    print("Narrator: Good game, and thanks for saving our town!")
...
# state who's turn it is
    ...
    print("That wasn't a good move. Not a proper response. Restart the game. :(")
